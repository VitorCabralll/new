 ...
 * POST /api/generate
 *
 * Geração com sistema de agentes unificado
 */
router.post('/generate', upload.single('file'), validateUpload, async (req: any, res: any) => {
  ...
  try {
    ...
    console.log('\n[Generate] ===== INÍCIO DO PROCESSAMENTO =====');
    ...
    if (cachedResult) {
      console.log(`[Generate] ✓ Cache hit! MD5: ${fileMD5}`);
    ...
    console.log(`[Generate] ✗ Cache miss, processando...`);
    ...
    console.log(`[Generate] UserAgent: ${userAgent.name} (${userAgent.category})`);
    ...
    // 3. EXTRAÇÃO DE TEXTO
    console.log('[Generate] Fase 1: Extração de texto');
    ...
    console.log(`[Generate] ✓ Texto extraído: ${textoCompleto.length} caracteres`);

    // 4. ANÁLISE UNIVERSAL
    console.log('[Generate] Fase 2: Análise Universal');
    ...
    console.log(`[Generate] ✓ Análise concluída: Tipo=${analise.tipoDocumento}`);

    // 5. PLANEJAMENTO UNIVERSAL
    console.log('[Generate] Fase 3: Planejamento Universal');
    ...
    console.log(`[Generate] ✓ Plano criado: ${plano.estrutura.length} seções`);

    // 6. GERAÇÃO COM FEW-SHOT LEARNING
    console.log('[Generate] Fase 4: Geração Few-Shot');
    ...
    while (iteracao < MAX_ITERACOES) {
      console.log(`[Generate] Fase 5.${iteracao + 1}: Avaliação (Iteração ${iteracao + 1}/${MAX_ITERACOES})`);
      ...
      console.log(`[Generate] ✓ Score: ${scoreAtual}/10`);

      // Verificar se atingiu qualidade
      if (scoreAtual >= SCORE_MINIMO) {
        console.log(`[Generate] ✓ Qualidade atingida! (${scoreAtual} >= ${SCORE_MINIMO})`);
        break;
      }

      // Se última iteração, aceitar
      if (iteracao >= MAX_ITERACOES - 1) {
        console.log(`[Generate] ⚠ Limite de iterações. Score final: ${scoreAtual}`);
        break;
      }

      // Refinar
      console.log(`[Generate] Fase 6.${iteracao + 1}: Refinamento`);
      ...
      console.log(`[Generate] ✓ Refinamento concluído`);
    ...
    console.log(`[Generate] ✓ Resultado cacheado`);
    ...
    try {
      ...
      console.error('[Generate] Erro ao limpar arquivo:', err);
    }

    console.log('[Generate] ===== PROCESSAMENTO CONCLUÍDO =====\n');
